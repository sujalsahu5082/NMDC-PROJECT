...
LOGIN_PAGE   = BASE_DIR.parent / 'nmdc_login.html'  # root login page
USERS_FILE   = BASE_DIR.parent / 'Users.xlsx'        # user credentials file
...
NONPROD_DEPTS = ['Civil','Materials','T&S and Environment','Finance','Human Resource',
...
def normalize_department(raw):
    ...
    for d in ALL_DEPTS:
        if d.lower() == s.lower():
            return d
    sl = s.lower()
    # Handle actual department names from the NMDC Excel file and map old summary names
    ...
    if 'cgm (p)' in sl or 'gm (p)' in sl:                  return 'GM (P) Sectt.'
    # The summary Excel uses 'C & IT', but the sidebar uses 'Commercial'
    if 'commercial' in sl or 'c &' in sl or ('c' in sl and 'it' in sl): return 'Commercial'
...
# Non-production departments shown individually in the sidebar
...
def build_departments_dist_from_summary(summary, dept_filter='All', cat_filter='All'):
    ...
    # Iterate over all known departments to ensure we don't miss any (e.g. CSR)
    # that might be absent from the summary Excel.
    for dept in ALL_DEPTS:
        if cat_filter != 'All' and get_category(dept) != cat_filter:
            continue
        
        dept_data = summary['departments'].get(dept, {})
...
def load_users():
    """
    Read Users.xlsx fresh every time so any edits to the file
    (new users, password changes, role changes) are picked up immediately
    without restarting the server.
    Returns a list of dicts with keys: empid, usertype, user, password.
    """
    if not USERS_FILE.exists():
        app.logger.warning('Users.xlsx not found at %s', USERS_FILE)
        return []
    try:
        df = pd.read_excel(str(USERS_FILE), dtype=str)
        # Normalise column names to lowercase with no extra spaces
        df.columns = [c.strip().lower() for c in df.columns]
        users = []
        for _, row in df.iterrows():
            users.append({
                'empid':    str(row.get('empid', '')).strip(),
                'usertype': str(row.get('usertype', '')).strip().lower(),
                'user':     str(row.get('user', '')).strip(),
                'password': str(row.get('password', '')).strip(),
            })
        return users
    except Exception as exc:
        app.logger.error('Failed to read Users.xlsx: %s', exc)
        return []


def validate_credentials(usertype, username, password):
    """
    Validate login credentials against Users.xlsx.
    usertype : 'admin' or 'employee' (case-insensitive)
    username : matched against either the USER column (name) OR the EMPID column
    Returns the matching user dict on success, None on failure.
    """
    users = load_users()
    ut    = usertype.strip().lower()
    ...
    for u in users:
        ...
        if u['password'] != password:
            continue
        # Accept login if either the display name OR the employee-ID matches
        name_match  = u['user'].lower() == query
        empid_match = u['empid'].lower() == query
        if name_match or empid_match:
            return u
    return None
...
@app.route('/login')
def login_page():
    """Serve the NMDC login page (always accessible)."""
    return send_file(str(LOGIN_PAGE))


@app.route('/dashboard')
def index():
    """Serve the main HR Analytics dashboard — requires a valid login session."""
    if not session.get('logged_in'):
        return redirect('/login')
    return send_file(str(FRONTEND_DIR / 'index.html'))


@app.route('/NMDC LOGO.jpg')
def nmdc_logo():
    """Serve the NMDC logo image from the project root."""
    return send_file(str(BASE_DIR.parent / 'NMDC LOGO.jpg'))


@app.route('/api/login', methods=['POST'])
def api_login():
    """
    Authenticate a user against Users.xlsx.
    Body JSON: { usertype, username, password }
    Returns 200 + { success, user, usertype, empid } on success.
    Returns 401 + { error } on failure.
    """
    body     = request.get_json(force=True) or {}
    usertype = body.get('usertype', '').strip()
    ...
    user = validate_credentials(usertype, username, password)
    if user is None:
        return jsonify({'error': 'Invalid credentials. Please check your User Type, Username and Password.'}), 401

    # Store minimal info in the Flask session (server-side cookie)
...
def api_chat():
    """
    Handle user queries in the chatbot.
    Receives: query (str), data_summary (dict), and optional api_key (str)
    """
    body = request.get_json(force=True) or {}
    ...
    try:
        from chat_agent import AIChatAgent
        agent = AIChatAgent()
        response_text = agent.handle_query(query, data_summary, api_key)
